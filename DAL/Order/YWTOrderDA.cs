...
namespace YWT.DAL.Order
{
    ...
    public class YWTOrderDA
    {
        ...
        /// <param name="mResultMessage"></param>
        /// <returns></returns>
        public List<YWTOrderPlatformOR> PlatformOrderListSearch(int StartIndex, int endIndex, out int mResultType, out string mResultMessage)
        {
            ...
            {
                List<YWTOrderPlatformOR> _list = new List<YWTOrderPlatformOR>();
                foreach (DataRow _row in ds.Tables[0].Rows)
                {
                    _list.Add(new YWTOrderPlatformOR(_row));
                }
                return _list;
            }
            return null;
        ...
        /// <param name="mResultMessage"></param>
        /// <returns></returns>
        public YWTOrderPlatformOR PlatformOrderItemSearch(string Order_ID, string Create_User, int endIndex, out int mResultType, out string mResultMessage)
        {
            string sql = @"SP_YWTOrder_Platform_Search";
            SqlParameter[] parameters = new SqlParameter[]
			{
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                YWTOrderPlatformOR result = null;
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataRow _row = ds.Tables[0].Rows[0];
                    result = new YWTOrderPlatformOR(_row);
                }
                return result;
            }
            return null;
